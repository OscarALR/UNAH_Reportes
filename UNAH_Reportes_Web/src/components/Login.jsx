        ...
        <section className={`login-acceso${esRegistro ? ' registro-activo' : ''}`}>
          <div className={`login-acceso-contenido${esRegistro ? ' login-registro-contenido' : ''}`}>
            ...
            <form className="login-local-form" onSubmit={enviarFormularioLocal}>
              ...
              {modoLocal !== 'recuperar' && <label>Contraseña<span className="campo-contrasena"><input required type={mostrarContrasena ? 'text' : 'password'} minLength="8" placeholder={modoLocal === 'registro' ? 'Crea una contraseña segura' : 'Ingresa tu contraseña'} value={formulario.contrasena} onChange={(e) => setFormulario({ ...formulario, contrasena: e.target.value })} /><button type="button" onClick={() => setMostrarContrasena((visible) => !visible)} aria-label={mostrarContrasena ? 'Ocultar contraseña' : 'Mostrar contraseña'}><FontAwesomeIcon icon={mostrarContrasena ? faEyeSlash : faEye} /></button></span>{modoLocal === 'registro' && <small>8+ caracteres, combinando letras, números y símbolos.</small>}</label>}
